...
const PlinkoGame = require('../model/plinko_gameV2');
const PlinkoHistory = require('../model/plinko_game_history');
const PlinkoEncrypt = require('../model/plinko_encryped_seeds');
...
const handleUpdatewallet = async (data, emitter) => {
  ...
  if (data.token === 'PPF') {
    let sjj = await PPFWallet.find({ user_id: data.userId });
    ...
    let sjj = await USDTWallet.find({ user_id: data.userId });
    prev_bal = parseFloat(sjj[0].balance);
