    ...
    const handleGrabCoinDrop = (async (data) => {
        ...
        if (coin_drop_participant.length + 1 <= coin_drop_num) {
            ...
            if (!userExists) {
                ...
                io.emit('grabCoinDropResponse', { data: coin_drop, message: "Coin drop grabbed successfully" });
                // Notify other participants about the new participant
            } else {
                io.emit('grabCoinDropResponse', { message: "User already exists in coin drop participants" });
                return;
            }
        } else {
